                            ...
                            foreach (var cdfn in worksheetData.Comments)
                            {
                                if (cdfn != null)
                                {
                                    ...
                                    if (worksheetData.CommentShapeXml == null)
                                    {
                                        ...
                                        string anchor = (cdfn.Anchor != null)
                                            ? cdfn.Anchor.ToString()
                                            : cdfn.CreateAnchor().ToString();
                                        shapeXml = string.Format("<v:shape id =\"{0}\" type=\"#_x0000_t202\" style=\"{1}\" fillcolor=\"#ffffe1\" o:insetmode=\"auto\"><v:fill color2=\"#ffffe1\"/><v:shadow color=\"black\" obscured=\"t\"/><v:path o:connecttype=\"none\"/><v:textbox style=\"mso-direction-alt:auto\"><div style=\"text-align:left\"></div></v:textbox><x:ClientData ObjectType=\"Note\"><x:MoveWithCells/><x:SizeWithCells/><x:Anchor>{2}</x:Anchor><x:AutoFill>False</x:AutoFill><x:Row>{3}</x:Row><x:Column>{4}</x:Column></x:ClientData></v:shape>",
                                            shapeId, shapeStyle, anchor, shapeRow, shapeCol);
                                    }
                                    else
                                    {
                                        shapeXml = worksheetData.CommentShapeXml(shapeId, cdfn);
                                    }
